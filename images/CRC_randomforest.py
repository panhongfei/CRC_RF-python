...
def load_csv(filename):  # 导入csv文件
    dataset = list()  # 创建一个空列表
    with open(filename, 'r') as file:
        csv_reader = reader(file)  # reader 按行读取csv文件,每一行组成一个list的元素
        for row in csv_reader:
            if not row:
                continue
            dataset.append(row)  # 每次添加一行
    return dataset
...
def str_column_to_int(dataset, column):  # 将最后一列表示标签的值转换为Int类型0,1,...
    class_values = [row[column] for row in dataset]  # 将每行的第column个数放入到class_values中
    unique = set(class_values)  # 去掉列表中重复值
    lookup = dict()  # 创建一个空字典
    for i, value in enumerate(unique):  # enumerate(列表)可以得到列表以及列表中内容的编号
        lookup[value] = i  # 字典中每个值都有一个编号了即 {'value1':0,'value2':1,...}
    for row in dataset:
        row[column] = lookup[row[column]]  # 根据字典规则把每一行第column列的值换为编号
    return lookup


# Split a dataset into k folds
def cross_validation_split(dataset,
                           n_folds):  # cross_validation:交叉验证.将数据集dataset分成n_flods份，每份包含len(dataset) / n_folds个值，每个值由dataset数据集的内容随机产生，每个值被使用一次
    dataset_split = list()
    ...
    for i in range(n_folds):
        ...
        while len(fold) < fold_size:  # 这里不能用if，if只是在第一次判断时起作用，while执行循环，直到条件不成立
            ...
            fold.append(dataset_copy.pop(
                index))  # 将对应索引index的内容从dataset_copy中导出，并将该内容从dataset_copy中删除。pop() 函数用于移除列表中的一个元素（默认最后一个元素），并且返回该元素的值。
...
def test_split(index, value, dataset):
    ...
    for row in dataset:
        if row[index] < value:  # 每次比较的都是各行的第index个值,相当于我们这里的第index个微生物,酶一个row相当于一个样本(患者)
            left.append(row)
        else:
            right.append(row)
    return left, right
...
def gini_index(groups, class_values):  # 个人理解：计算代价，分类越准确，则gini越小
    ...
    for class_value in class_values:  # class_values =[0,1]
        for group in groups:  # groups=(left,right)
            ...
            proportion = [row[-1] for row in group].count(class_value) / float(
                size)  # str.count(sub, start= 0,end=len(string)),
            # sub -- 搜索的子字符串,
            # start -- 字符串开始搜索的位置。默认为第一个字符,第一个字符索引值为0。
            # end -- 字符串中结束搜索的位置。字符中第一个字符的索引为0。默认为字符串的最后一个位置。
            # 因此,这一句的意思是,group 中class_values的个数比上group内样本的个数
            gini += (proportion * (1.0 - proportion))  # Gini=1-(p1^2+p2^2+....pn^2)  当n=2 时Gini=2p(1-p)
...
def get_split(dataset, n_features):
    ...
    features = list()
    while len(features) < n_features:  # 这一步需要改进
        ######################################################################################################################################
...
n_features = 27
for n_trees in [1,10,50,100]:  # 理论上树是越多越好
